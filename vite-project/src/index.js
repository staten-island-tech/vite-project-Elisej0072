...
function filterByCollection() {
  const button = document.querySelector(".filtercollection");
  button.addEventListener("click", function(){
  document.querySelector(".filtered").innerHTML = ""
  collection.forEach((item)=>   document.querySelector(".filtered").insertAdjacentHTML("afterbegin", 
  `<h2>${item.name} </h2>
  <img src="${item.image}">
  <h4> $${item.price} </h4>`
 ))
...
function filterByWishlist() {
 const button = document.querySelector(".filterwishlist");
 button.addEventListener("click", function(){
  document.querySelector(".filtered").innerHTML = ""
  wishlist.forEach((item)=>   document.querySelector(".filtered").insertAdjacentHTML("afterbegin", 
    `<h2>${item.name} </h2>
     <img src="${item.image}">
  <h4> $${item.price} </h4>`
  ))
